...
export async function generateNode(
  ...
  const { generatorMaxTokens } = loadLlmConfig();
  ...
  for (let i = 0; i < orderedModules.length; i++) {
    ...
    try {
      const response = await llm.call(
        ...
        { jsonMode: true, temperature: 0.1, maxTokens: generatorMaxTokens },
      );

      const parsed = JSON.parse(response);
      const alreadyGeneratedPaths = new Set(allFiles.map((f) => f.relativePath));
      const generatedFiles: GeneratedFile[] = (parsed.files ?? [])
        .filter((f: any) => {
          if (alreadyGeneratedPaths.has(f.relativePath)) {
            logger.warn(`Skipping duplicate file: ${f.relativePath} (already generated by a previous module)`);
            return false;
          }
          return true;
        })
...
async function getLegacyContext(
  ...
  try {
    ...
    return promptBuilder.buildContext([...codeResults, ...ruleResults], 3500);
  } catch {
    return '';
  }
}
